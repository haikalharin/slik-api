...
export class MasterController {
    ...
    async getAllValidation() {
        const masterValidations = await this.masterService.findAllMasterValidation();

        try {
            return ResponseUtil.success(masterValidations);
        } catch (err) {
            return ResponseUtil.error(err, HttpStatus.BAD_REQUEST);
    ...
    async createValidation(@Body() createValidationDto: Partial<MasterValidation>) {
        try {
            Logger.log(createValidationDto.validationName);
            const listCity = await this.masterService.findAllMasterValidation();
            if (listCity.length !== 0) {
                const cityExists = await this.masterService.checkUserExistsByValidationName(createValidationDto.validationName)
                if (cityExists) {
                    return ResponseUtil.error('User Exist', HttpStatus.CONFLICT);

                } else {
                    const data = await this.masterService.createValidation(createValidationDto);
                    return ResponseUtil.success(data);
                }
            } else {
                const data = await this.masterService.createValidation(createValidationDto);
